...
// Node represents a network validator
type Node struct {
	ID       int
...
func H1(block int64) *bn256.G1 {
	h := sha256.New()
	h.Write(big.NewInt(block).Bytes())
...
func Setup(n, t int) ([]*Node, *Params, error) {
...
// Encrypts a transaction for a given block
func Encrypt(params *Params, msg []byte, block int64) (*Transaction, error) {
...
// ComputeShare computes a node's share for an block
func (k *Node) ComputeShare(block int64) *bn256.G1 {
	h1i := H1(block)
	return new(bn256.G1).ScalarMult(h1i, k.MskShare)
}

// CombineShares combines t+1 shares to recover the block secret key
...
// Decrypt decrypts a transaction using the block secret key
...
func main() {
	...
	// Generate nodes and system parameters
	fmt.Println("\n1. Setting up system parameters and nodes...")
	nodes, params, err := Setup(numNodes, threshold)
	if err != nil {
		log.Fatalf("Setup failed: %v", err)
	}
	fmt.Printf("Successfully generated %d nodes\n", len(nodes))
	...
	// Encrypt message
	fmt.Println("\n2. Encrypting message for block", block)
	tx, err := Encrypt(params, testMessage, block)
	if err != nil {
	...
	// Generate shares for block
	fmt.Println("\n3. Generating node shares for block", block)
	shares := make(map[int]*bn256.G1)
	for _, node := range nodes {
		share := node.ComputeShare(block)
		shares[node.ID] = share

		// Print first few bytes of each share
		shareBytes := share.Marshal()
		fmt.Printf("Node %d share : %x\n",
			node.ID,
			shareBytes[:])
	}

	// Combine shares
	fmt.Println("\n4. Combining shares to get block secret key...")
	sk := CombineShares(shares)
	skBytes := sk.Marshal()
	fmt.Printf("Block secret key: %x\n", skBytes[:])
	...
	for i := 0; i < threshold-1; i++ {
		insufficientShares[nodes[i].ID] = shares[nodes[i].ID]
	}
